$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε την χορήγηση ετήσιας εισοδηματικής ενίσχυσης σε οικογένειες ορεινών και μειονεκτικών περιοχών!"
        : "We are sorry but you are not eligible to receive the annual income support for families in mountainous and disadvantaged areas!";
  ...
  function retrieveAnswers() {
    ...
    if (allAnswers[5] === "1") {
      currentLanguage === "greek"
      ? setResult(
          "Δικαιούστε ενίσχυση 600€ ετησίως."
        )
      : setResult(
          "You are eligible for a support of €600 per year."
        );
    } else if (allAnswers[5] === "2" ) {
        currentLanguage === "greek"
          ? setResult(
              "Δικαιούστε ενίσχυση 300€ ετησίως."
            )
          : setResult(
              "You are eligible for a support of €300 per year."
            );
      } 
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να λάβετε χορήγηση ετήσιας εισοδηματικής ενίσχυσης σε οικογένειες ορεινών και μειονεκτικών περιοχών είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive the provision of annual income support to families in mountainous and disadvantaged areas are following:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        ...
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd(" Η ενίσχυση απευθύνεται μόνο σε οικογένειες, σύμφωνα με τα καθορισμένα κριτήρια.") : skipToEnd("You do not meet the criteria for receiving the support. The support is only available to families, according to the established criteria.");
      }else if (currentQuestion === 3 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Η ενίσχυση αφορά μόνο περιοχές που είναι είτε ορεινές είτε μειονεκτικές.") : skipToEnd("The support is only available for areas that are either mountainous or disadvantaged.");
      }  else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Για να δικαιούστε την ενίσχυση, απαιτείται συνεχής διαμονή στην περιοχή για τουλάχιστον δύο (2) χρόνια") : skipToEnd("To be eligible for the support, continuous residence in the area for at least two (2) years is required.");
      } else if (currentQuestion === 5 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Το εισόδημά σας υπερβαίνει τις 4.700 €, δεν πληροίτε τα εισοδηματικά κριτήρια για την ενίσχυση.") : skipToEnd("Your income exceeds €4,700; you do not meet the income criteria for the support.");
      } else if (currentQuestion === 6 && selectedRadioButtonIndex === 5) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Απαιτείται η ιδιότητα πολίτη της Ελλάδας ή ενός κράτους-μέλους της Ευρωπαϊκής Ένωσης, του Ευρωπαϊκού Οικονομικού Χώρου ή της Ελβετίας.") : skipToEnd("You do not meet the requirements to submit an application, as citizenship of Greece or a European Union Member State, the European Economic Area, or Switzerland is required.");
      } else {
